...
**6. Checklist (JSON):** `<details class="collapsible-section"><summary><h2 id="learning-checklist">☑️ Learning Checklist</h2></summary><div class="section-content"><div class="checklist-progress"><p>Progress: <span id="checklistProgress">0/15</span></p><div class="progress-bar"><div class="progress-fill" id="progressFill" style="width: 0%;">0%</div></div></div><div id="checklistContainer" data-checklist-source="json/checklist.json"></div><div class="quiz-reset-container"><button onclick="resetChecklist()" class="quiz-reset-btn">🔄 Reset Checklist</button></div></div></div></details>`

**7. Reflection Questions:** `<details class="collapsible-section"><summary><h2 id="reflection-questions">🤔 Reflection Questions</h2></summary><div class="section-content"><div class="question-block"><h4>1. [Question]</h4><details class="hint-details"><summary class="hint-summary">💡 Hint</summary><div class="hint-content"><p>[Hint text]</p></div></details></div><!-- 5-8 questions --></div></details>`
...
```
Introduction (collapsible — FIRST section, same as standard notes, JS opens it automatically)

Background / Theory (collapsible — multi-section, covers all theory needed for the lab, extended tasks, and projects)
  Each sub-topic is its own nested collapsible <details> inside the Background section
  Example: VPC Peering lab → separate dropdowns for "What is VPC Peering?", "Why Use VPC Peering?", "When to Use It?", "How Does It Work?"
  Covers: what, why, when, how — "where" is explained naturally inside the tasks themselves

Lab Overview (collapsible — placed AFTER Background / Theory)
...
**Hands-On Projects (in labs):**
- ❌ Not generated by default
- ✅ Only if user explicitly asks: "include projects" or "include hands-on projects"
- Same name, same structure, same depth as Hands-On Projects in standard notes
- Each project inside the section is an h3 heading — never h2
- Projects live inside the lab body, not as an end section
...
Labs include theory but keep it focused:
- Background section — covers what the technology is, why it exists, when to use it, and how it works internally; "where" is naturally covered inside the tasks themselves
- Each sub-topic in Background gets its own nested collapsible — never one long wall
- Inline context at the start of each task — 1-2 sentences connecting the task to real-world use
- No long concept walls inside tasks — deep theory belongs in the Background section or in the standard notes for that topic
